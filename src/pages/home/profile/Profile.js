...
const Profile = () => {
  ...
  // } = user;
  const sectorsOfInvestingArray =
    user?.sectorsOfInvesting === "Lorem Ipsum" ? [] : user?.sectorsOfInvesting;
  ...
  const onSaveChangesHandler = async () => {
    ...
    if (true) {
      ...
      await updateUserInDatabse(user?.uid, updatedData);
  ...
  const onEditClickHandler = () => {
    ...
    setExperience(user?.experienceOfInvesting);
    setStage(user?.stageOfInvestment);
    setSector(user?.sectorsOfInvesting);
    setFrom(user?.country);
    setAmt(user?.amount);
    setDescription(user?.userDescription);
    // setType(userType);
    setLinkedUrl(user?.linkedInUrl);
    setWhenToInvest(user?.whenToStartInvesting);
    setInstagramUrl(user?.instagram);
    setTwitterUrl(user?.twitter);
    setUserImgFile(user?.userImg);
  };
              ...
              <>
                ...
                <div className="profile__social">
                  <div className="profile__stats ">
                    <h4>Sectors for Investment</h4>
                    <h5>
                      {sectorsOfInvestingArray?.map((sector) => (
                        <sapn
                          style={{
                    ...
                    <h4>Preferred stage for Investment</h4>
                    <h5>{user?.stageOfInvestment}</h5>
                    <h4>Amount you want Invest </h4>
                    <h5>{user?.amount}</h5>
                    <h4>By when do you want to start Investing </h4>
                    <h5>{user?.whenToStartInvesting}</h5>
                    <h4>Years of experience in Investing </h4>
                    <h5>{user?.experienceOfInvesting}</h5>
                  </div>
